...
from handsoff.models.stylegan2_pytorch.stylegan2_pytorch import Generator as Stylegan2Generator
from handsoff.models.stylegan1_pytorch.stylegan1 import G_mapping, Truncation, G_synthesis
from handsoff.models.classifer import label_generator_mlp
...
def latent_to_image(g_all, upsamplers, latents, return_upsampled_layers=False, use_style_latents=False,
    ...
    if return_upsampled_layers:
        affine_layers_upsamples = torch.FloatTensor(1, number_features, dim, dim).cuda()
    ...
    return img_list, affine_layers_upsamples, number_features
...
def run_embedding_optimization(args, g_all, upsamplers, inter, percept, img_tensor, latent_in,
    ...
    org_latent_in = copy.deepcopy(latent_in.detach())
    with torch.no_grad():
        im_out_wo_opti, _ = latent_to_image(g_all, upsamplers, org_latent_in, process_out=False,
                                            dim=args['im_size'][1],
    ...
    for _ in tqdm(range(1, steps)):
        ...
        if regular_by_org_latent:
            encoder_loss = F.mse_loss(latent_in, org_latent_in)
        else:
...
def prepare_model(args, classfier_checkpoint_path="", classifier_iter=10000, num_class=34, num_classifier=10):
    ...
    if args['domain'] == 'face' or args['domain'] == 'flickr_car':
        res = 1024
        out_res = 512
    elif args['domain'] == 'face_256':
        res = 256
        out_res = 256
    elif args['domain'] == 'cityscapes':
        res, out_res = 512, 512
    else:
        res = 512
        out_res = 512
    ...
    if args['stylegan_ver'] == "1":
        if args['domain'] == "car":
            ...
            max_layer = 8
        elif args['domain'] == "bedroom":
            max_layer = 7
        elif args['domain'] == "cat":
            max_layer = 7
        else:
        ...
        print("Load StyleGAN2 from, " , args['stylegan_checkpoint'], " at res, ", str(res))
    ...
    if classfier_checkpoint_path != "":
        ...
        for MODEL_NUMBER in range(num_classifier):
            classifier = label_generator_mlp(num_class, dim=args['dim'])
            classifier = nn.DataParallel(classifier, device_ids=device_ids).cuda()
            if classifier_iter > 0:
                checkpoint = torch.load(
                    os.path.join(classfier_checkpoint_path,
                                'car_model_iter' + str(classifier_iter) + '_number_' + str(MODEL_NUMBER) + '.pth'))
            else:
